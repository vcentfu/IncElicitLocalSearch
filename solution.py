...
def true_sol(d, dm, object_key = 'i', weight_bag = 'W'):
    ...
    m.addConstr(sum([d[object_key][j][0] * x[j] for j in range(len(x))]) <= d[weight_bag], "bag_c")
    ...
    if dm.type_a == "LW":
        ...
        for l in range(len(dm.omega)):
            for u in range(l + 1, len(dm.omega)):
                M.append(sum([abs(d[object_key][k][l + 1] - d[object_key][k][u + 1]) for k in range(len(d[object_key]))]))
